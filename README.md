...
To enable native module support, start your script with a `!native` directive on the first line. This allows the use of `runtime.loadlib` to load pre-compiled native modules (`.dll`, `.so`, etc.), which can contain high-performance code, including JIT-compiled functions or hardware-accelerated operations.
...
- `runtime.native_enabled`: A boolean indicating if native library loading is enabled.
